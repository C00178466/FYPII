...
void Game::Init()
{
	char* argv[1];
	int argc = 1;
	argv[0] = _strdup("Terrain Generation");
	glutInit(&argc, argv);
...
void Render()
{
	...
	float scale = 6.0f / max(terrain->width() - 1, terrain->length() - 1);
...
void UpdateRotation(int value)
{
	...
	glutTimerFunc(25, UpdateRotation, 0); // Call Update every 25 milliseconds

	glutDisplayFunc(Render);
	glutKeyboardFunc(HandleInput);
	glutReshapeFunc(HandleResize);
	glutTimerFunc(25, UpdateRotation, 0);
